...
export class DoctorsService extends UnsubscribeOnDestroyAdapter {
  ...
  private readonly API_URL_AVERAGE_RATING = 'http://localhost:8085/Examen/realEstateAgents'; // URL to fetch average rating
  ...
  addDoctor(doctor: Doctors): Observable<any> {  // Ensure to return Observable
    return this.httpClient.post(this.API_URL_ADD, doctor);
  }
  ...
  getAverageRating(agentId: number): Observable<number> {
    return this.httpClient.get<number>(`${this.API_URL_AVERAGE_RATING}/${agentId}/average-rating`);
  }
